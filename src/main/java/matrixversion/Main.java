...
public class Main {
    ...
    public static List<Integer> nodesNumber = new ArrayList<>();
    public static int[][] shortestMatrixPlus;
...
//        pathGenerator();
    ...
    //对单词进行过滤统计
    public static List<String> readFileAndExtractWords(String fileName) {
    ...
    //用来构建图的二维矩阵
    public static void textToGraph(String filePath) {
        //用一个map来存储每一个单词以及图的大小
        Map<String, Integer> nodesMap = new HashMap<>();
        List<String> words = readFileAndExtractWords(filePath);
        nodeGraph = new Graph(new ArrayList<>(), words.size());
        
        //进行map填充
        for(int i = 0; i < words.size(); i++) {
            String word = words.get(i);
            if(i == 0) {
                nodeGraph.addNodes(new Node(word));
                //首先对第一个元素进行添加
                ...
                nodeGraph.addEdges(nodeGraph.getNodesSize() - 1, nodesMap.get(word));
    ...
    public static String generateNewText(String inputText) {
        ...
        String[] wordsArray = inputText.toLowerCase().split("\\W+");
        for (String s : wordsArray) {
            if(s != null) {
                inputWords.add(s);
            }
        ...
        for(int i = 0; i < inputWords.size() - 2; i++) {
            List<String> bridgeList = queryBridgeWords(inputWords.get(i), inputWords.get(i + 1));
    ...
    //调用python脚本
     ...
     private static void callPythonScript(String nodesFile, String matrixFile) {
        try {
            ProcessBuilder pb = new ProcessBuilder("python", "E:\\@python-code\\sf_en_lab1\\generate_graph_plus.py", nodesFile, matrixFile, "output.svg");
            pb.inheritIO();
    ...
    //调用python脚本
     @SuppressWarnings("CallToPrintStackTrace")
     private static void callPythonScript(String nodesFile, String matrixFile, String nodesFileShortest, String matrixFileShortest) {
        try {
            ProcessBuilder pb = new ProcessBuilder("python", "E:\\@python-code\\sf_en_lab1\\generate_graph_shortest.py", nodesFile, matrixFile, nodesFileShortest, matrixFileShortest, "output.svg");
            pb.inheritIO();
            Process process = pb.start();
            process.waitFor();
        } catch (IOException | InterruptedException e) {
    ...
    //生成有向图
    public static void graphGenerator() {
        int[][] matrix = nodeGraph.getEdges();
        Node[] nodes = nodeGraph.getNodes().toArray(new Node[0]);
        String matrixFile = "matrix.txt";
        String nodesFile = "node.txt";
    ...
    //生成最短路径的图
    public static void pathGenerator() {
//        getShortestNodes();
        int[][] matrix = nodeGraph.getEdges();
        Node[] nodes = nodeGraph.getNodes().toArray(new Node[0]);
        String matrixFile = "matrix.txt";
        ...
        Node[] pathNodes = shortestPathGenerator();
        shortestGraph(pathNodes);
        String nodesFileShortest = "nodeShortest.txt";
        String matrixFileShortest = "matrixShortest.txt";
        writeMatrixToFile(shortestMatrixPlus, matrixFileShortest);
        writeNodesToFile(pathNodes, nodesFileShortest);
        
        callPythonScript(matrixFile, nodesFile, nodesFileShortest, matrixFileShortest);
    }
    
    //根据获取的最短路径来对边矩阵进行修改
    public static void shortestGraph(Node[] pathNodes) {
        int[][] shortestMatrix = nodeGraph.getEdges();
        shortestMatrixPlus = new int[nodeGraph.getNodesSize()][nodeGraph.getNodesSize()];
        for (int i = 0; i < nodeGraph.getNodesSize(); i++) {
            for (int j = 0; j < nodeGraph.getNodesSize(); j++) {
                shortestMatrix[i][j] = 0;
            }
        }
        for (int i = 0; i < nodesNumber.size(); i++) {
            shortestMatrixPlus[nodesNumber.get(i)][nodesNumber.get(i + 1)] = shortestMatrix[nodesNumber.get(i)][nodesNumber.get(i + 1)];
        }
    }
    
    //生成最短路径
    public static Node[] shortestPathGenerator() {
        Node[] pathNodes = new Node[nodesNumber.size()];
        for (int i = 0; i < nodesNumber.size(); i++) {
            pathNodes[i] = nodeGraph.getNodes().get(nodesNumber.get(i));
        }
        return pathNodes;
    }
    
    //获取目标路径之上的节点
    public static void getShortestNodes(int[] predecessors, int currentVertex) {
        if(currentVertex == -1) {
            return;
        }
        getShortestNodes(predecessors, predecessors[currentVertex]);
        nodesNumber.add(currentVertex);
    }
    ...
    public static void dijkstra(int sourceVertex) {
        ...
        for (int v = 0; v < numberOfVertices; v++) {
                if(v != sourceVertex) {
                    System.out.println("Shortest distance from " + sourceVertex + " to " + v + " is: " + distances[v]);  
                    System.out.print("Shortest path from " + sourceVertex + " to " + v + " is: ");  
                    printPath(predecessors, v);
                    if(v == 1) {
                        getShortestNodes(predecessors, v);
                    }
                    System.out.println();
                }
            }
    ...
    //打印到目标节点的最短路径
    public static void printPath(int[] predecessors, int currentVertex) {
        if(currentVertex == -1) {
            return;
        } 
    ...
    public static void randomWalk(Node startNode) {
        Set<String> visitedEdges = new HashSet<>(); //当前遍历过的边的信息,要求是不能够重复
        List<Node> visitedNodes = new ArrayList<>(); //当前遍历过的节点,是可以重复的
    ...
    //从图中随机选择一个节点
    public static Node getRandomNode(List<Node> nodeList) {
        Random random = new Random();
        return nodeList.get(random.nextInt(nodeList.size()));
    }
    
    //遍历图的节点
    public static void traverse(Node node, Set<String> visitedEdge, List<Node> visitedNodes) {
        ...
        //先随机选择一个节点
        Node nextNode = getRandomNode(neighborNode);
        //判断是否存在重复边
